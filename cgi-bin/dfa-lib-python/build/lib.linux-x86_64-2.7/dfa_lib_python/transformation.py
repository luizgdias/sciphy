...
class Transformation(ProvenanceObject):
    ...
    def add_set(self, set):
        ...
        assert isinstance(set, Set), \
            "The set must be valid."
        print("Transformation add set")
        print(set.get_specification())
        self._sets.append(set.get_specification())

    def set_dependency(self, x):
        dependency_set = DependencySet(x.get_specification()["transformationTag"], x.get_specification()["tag"], x.get_specification()["type"])
        self._sets.append(dependency_set.get_specification())
