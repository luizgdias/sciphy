...
def ModulePhyml(dirin, model, bootstrap, nb_categ, alpha, kappa):
  btp = str(bootstrap)
  mod = str(model)
  kap = str(kappa)
  nbc = str(nb_categ)
  ...
  for f in os.listdir(dirin):
    if f.endswith('_phy'):                                                    
      ...
      #Phyml Parameters: Comparing and searching Modelgenerator evolutionary models for nucleotides
      #Attention: Phyml and Modelgenerator have different evolutionary models 
      modelPHYML = ['HKY','F84','TN93','GTR','custom','JC69','K2P','F81']
        ...
        #Pertencem a PHYML: modelPHYML, "This parameter sets the rate matrix for nucleotides data"
      ...
      for x in modelPHYML[:]:                                                       # make a slice copy of the entire list
        ...
        if m:
          ...
          cmd_phyml = "phyml_linux "+path_phylip_corrected+" 0 i 2 "+btp+" "+modelHit[1]+" "+kap+" e "+nbc+" "+alp+" BIONJ y n"
      ...
      if found is False:
        ...
        cmd_phyml = "phyml_linux "+path_phylip_corrected+" 0 i 2 "+btp+" "+modelPHYML[0]+" "+kap+" e "+nbc+" "+alp+" BIONJ y n"
        handle_phyml = os.popen(cmd_phyml, 'r', 1)
        for line_phyml in handle_phyml:
          print line_phyml,
        handle_phyml.close()
