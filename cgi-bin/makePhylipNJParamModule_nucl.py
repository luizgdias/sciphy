...
def ModulePhylipNJ(dirin, bootstrap):
  ...
  for f in os.listdir(dirin):
    if f.endswith('_phy'):                                                    
      ...
      #usando o modulo paramModule_nucl
...
def paramModulePhylipNJ(dirin, path_phylip_corrected, btp):
  ...
  #Attention: phylipNJ not contain consistency for evolutionary models for nucl (ia Modelgenerator)
  ...
  #3.- DNADIST
  #  Criando arquivo param_phylipDnadist
  arq_param_d = dirin + "/param_phylipDnadist" 
  d = file(arq_param_d, "w")
  d.write(path_phylip_corrected + "_phylipnj.seqboot" + '\n')
  d.write('M\n')
  d.write('D\n')
  d.write(btp + '\n')
  #F84, Kimura, Jukes-Cantor, LogDet
  d.write('Y\n')
  d.close()
  
  #  Executando parametros Dnadist
  print 'Executando Dnadist...\n'
  cmd_d = "phylip dnadist < " + arq_param_d
  handle_d = os.popen(cmd_d, 'r', 1)
  for line_d in handle_d:
    print line_d,
  handle_d.close()

  #  Mudando de nome e diretorio ao outfile do Dnadist
  cache_location2 = dirin + "/outfile"
  if not (cache_location2):
    os.mkdir(dirin + "/outfile")
    sh.move(dirin + "/outfile", path_phylip_corrected + "_phylipnj.dnadist")
  else: 
    sh.move(dirin + "/outfile", path_phylip_corrected + "_phylipnj.dnadist")
    print "exist"
  ...
  p.write(path_phylip_corrected + "_phylipnj.dnadist" + '\n')
  p.write('M\n')
  p.write(btp + '\n')
  p.write('431\n')
  p.write('Y\n')
